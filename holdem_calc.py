...
def parse_cards():
    ...
    # Go through the cards list two at a time, and make tuples out of hole cards
    hole_cards = []
    current_hole_cards = []
...
def generate_histogram_board(flat_board):
    histogram, condensed_board = [0] * 13, []
    for card in flat_board:
    ...
    for index, frequency in enumerate(histogram):
        if frequency != 0:
            val = index + 2
            condensed_board.append((val, frequency))
    ...
    return condensed_board, (current_max, max_val), (second_max, second_max_val)
...
# Print results
def print_results(hole_cards, stats_list, num_iterations, result_histograms):
    float_iterations = float(num_iterations)
    print "Winning Percentages:"
    for index, hole_card in enumerate(hole_cards):
        print hole_card, ": ", float(stats_list[index + 1]) / float_iterations
    print "Ties: ", float(stats_list[0]) / float(num_iterations), "\n"
...
def main():
    ...
    for i in xrange(num_iterations):
        ...
        stats_list[winner_index] += 1
        # Increment what hand each player made
        for index, result in enumerate(result_list):
            result_histograms[index][result[0]] += 1
    print_results(hole_cards, stats_list, num_iterations, result_histograms)
